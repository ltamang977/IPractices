package org.lv.ipractices.interviewcake_june;
...
public class Solution {
    ...
    public static List<Meeting> mergeRanges(List<Meeting> meetings) {
        ...
        Meeting old= meetings.get(0);

        for(int i=1; i<meetings.size(); i++){

            Meeting curr = meetings.get(i);
            //merge meetings
            if(old.getEndTime()>=curr.getStartTime()){
                //merge
                old = new Meeting(old.getStartTime(), Math.max(old.getEndTime(), curr.getEndTime() ));
